    ...
    try {
        ...
        if (!map_id && !mode_id) {
        return new Response(
            JSON.stringify({ error: 'At least one of map_id or mode_id must be provided' }),
            { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
        }
        ...
        return new Response(
            JSON.stringify({ error: `Cannot select map/mode on a ${match.status} match` }),
        ...
        return new Response(
            JSON.stringify({ error: `Cannot modify a ${matchMap.status} map slot` }),
            { status: 422, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
        }
        ...
        if (!modeData) {
            return new Response(
            JSON.stringify({ error: 'Mode not found' }),
            { status: 404, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            )
        }
        ...
        if (!modeData.is_active) {
            return new Response(
            JSON.stringify({ error: 'Mode is not active' }),
            { status: 422, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            )
        }
        }
        ...
        if (!linkData) {
            return new Response(
            JSON.stringify({ error: 'This map and mode combination is not valid' }),
            { status: 422, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
            )
        }
        }
