...
serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: match, error: matchError } = await adminClient
      .from("matches")
      .select(
        "id, status, contrib_id, match_format, comp_instance_id, game_title_id",
      )
      .eq("id", match_id)
      .single();
    ...
    if (match.match_format !== "battle_royale") {
      return new Response(
        JSON.stringify({
          error: "This function is only for Battle Royale matches",
        }),
        {
          status: 422,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (uniquePlacements.size !== results.length) {
      return new Response(
        JSON.stringify({
          error:
            "Duplicate placements found — each team must have a unique placement",
        }),
        {
          status: 422,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const participantIds = results.map(
      (r: { participant_id: string }) => r.participant_id,
    );
    ...
    const resultRows = results.map(
      (r: { participant_id: string; placement: number; kills: number }) => {
