...
async function ensureExternalPlayer(player: PandaPlayer, teamId: string) {
...
async function syncLineupForCompletedMatch(
  ...
  for (const [roster, teamId] of [
    ...
    for (const player of roster) {
      try {
        const externalPlayerId = await ensureExternalPlayer(player, teamId);
        const { error } = await supabase.from("external_match_lineups").upsert(
          {
            match_id: matchId,
            team_id: teamId,
...
async function getMatches(endpoint: string, limit = 10): Promise<PandaMatch[]> {
  ...
  if (!response.ok) {
    ...
    throw new Error(`PandaScore ${response.status} for ${url}: ${errorBody}`);
  }
...
async function ensureSerie(
  ...
  const { data, error } = await supabase
    ...
    .insert({
      ...
      name,
      slug,
      edition_label: serie.season ?? (serie.year ? String(serie.year) : null),
  ...
  try {
    ...
    for (const vm of validMatches) {
      ...
      if (
        ...
        tournamentsToResolve.set(vm.match.tournament.id, vm.match.tournament);
      }
    }
    ...
    await mapWithConcurrency(
      ...
      async (league) => {
        try {
          compSeriesIdByLeagueExtId.set(league.id, await ensureLeague(league));
        } catch (err) {
          console.error(
        ...
        try {
          const compSeriesId =
            serie.league_id != null
              ...
              : undefined;
          if (!compSeriesId)
            throw new Error(`No resolved league for serie ${serie.id}`);
          compInstanceIdBySerieExtId.set(
            serie.id,
            await ensureSerie(serie, compSeriesId, config.gameTitleId),
          );
        } catch (err) {
          console.error(
        ...
        try {
          const compInstanceId =
            tournament.serie_id != null
              ? compInstanceIdBySerieExtId.get(tournament.serie_id)
              : undefined;
          if (!compInstanceId)
            throw new Error(
        ...
        try {
          ...
          const compInstanceId =
            match.serie?.id != null
              ...
              : undefined;
          if (!compInstanceId)
            throw new Error("No resolved competition instance for this match");

          const stageId =
            match.tournament?.id != null
              ? (stageIdByTournamentExtId.get(match.tournament.id) ?? null)
              : null;
