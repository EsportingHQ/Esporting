...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!match_id || !corrected_event_id) {
      return json({ error: 'match_id and corrected_event_id are required' }, 400)
    }
    ...
    const { data: match, error: matchError } = await adminClient
      .from('matches')
      .select('id, status, contrib_id, match_format, team_home_id, team_away_id')
      .eq('id', match_id)
      .single()
    ...
    const hasAccess = typedMatch.contrib_id === user.id ||
...
async function validateReplacementEvent({
  ...
  if (['goal', 'own_goal', 'penalty_goal', 'penalty_miss', 'yellow_card', 'red_card'].includes(eventType)) {
    if (match.match_format !== 'head_to_head') return 'Football replacement events require a head_to_head match'
  ...
  if (['score_update', 'round_end', 'map_end', 'map_selected', 'mode_selected'].includes(eventType)) {
    if (!matchMapId) return `replacement_match_map_id is required for '${eventType}'`
  ...
  if (eventType === 'score_update') {
    ...
    if (value === null || value < 0) return 'replacement_value must be a non-negative number for score_update'
  }

  return null
}

async function recalculateMatchState(match: MatchRow, adminClient: AdminClient) {
...
async function hasShooterScoringEvents(matchId: string, adminClient: AdminClient) {
...
async function recalculateFootballScore(match: MatchRow, adminClient: AdminClient) {
  ...
  const homeMapsWon = (mapTotals ?? []).filter((map) => map.map_winner === 'home').length
  const awayMapsWon = (mapTotals ?? []).filter((map) => map.map_winner === 'away').length
  ...
  if (match.status === 'completed') {
    await adminClient
      ...
      .update({
        winner_team_id:
          homeGoals > awayGoals ? match.team_home_id :
          awayGoals > homeGoals ? match.team_away_id :
          null,
      })
      .eq('id', match.id)
  }
...
async function recalculateShooterMaps(match: MatchRow, adminClient: AdminClient) {
  ...
  for (const event of scoreEvents ?? []) {
    ...
    if (event.team_id === match.team_home_id) score.home = Number(event.value ?? 0)
    if (event.team_id === match.team_away_id) score.away = Number(event.value ?? 0)
  }
  ...
  const mapEndByMap = new Map<string, { meta: Record<string, unknown>; created_at: string | null }>()
  ...
  for (const map of maps) {
    ...
    const mapWinner = score.home > score.away ? 'home' : score.away > score.home ? 'away' : 'draw'
    ...
    if (updatePayload.status === 'live' || (!updatePayload.status && map.status === 'live')) {
      liveMapId = map.id
      displayScore = score
    }
  ...
  if (match.status === 'completed') {
    matchUpdate.winner_team_id =
      homeMapsWon > awayMapsWon ? match.team_home_id :
      awayMapsWon > homeMapsWon ? match.team_away_id :
      null
  }
...
async function getLastNonVoidEventId(matchId: string, adminClient: AdminClient) {
