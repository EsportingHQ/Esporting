  ...
  try {
    ...
    if (!series_id && !series_name) {
      return new Response(
        JSON.stringify({
          error:
            "Either series_id (existing series) or series_name (new series) is required",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const canCreate =
      userRoles.includes("super_admin") || userRoles.includes("organiser");
    if (!canCreate) {
      return new Response(
        JSON.stringify({
          error: "Only organisers and admins can create competitions",
        }),
        {
          status: 403,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    const foundSlugs =
      gameTitles?.map(
        (g: { id: string; slug: string; name: string }) => g.slug,
      ) ?? [];
    ...
    const brGames = (gameTitles ?? []).filter(
      (g: { id: string; slug: string; name: string }) =>
        brSlugs.includes(g.slug),
    );
    ...
    for (const brGame of brGames) {
      ...
      if (template && template.length > 0) {
        ...
        await adminClient.from("br_points_config").insert(configRows);
      }
    }
