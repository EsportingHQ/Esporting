            ...
            <div className="space-y-3 text-xs font-data">
              {matchMaps.map((slot) => {
                const gameInfo = scoreBreakdown?.games?.find(
                  (g) => Number(g.position) === Number(slot.map_number),
                );
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="block font-display font-bold uppercase tracking-wider text-[9px] text-text-muted">
                          MAP 0{slot.map_number} — {slot.status.toUpperCase()}
                        </span>

                        <span className="font-body font-semibold text-text-primary">
                          {slot.maps?.name ?? `Map ${slot.map_number}`}
                        </span>
              ...
              {matchMaps.length === 0 && scoreBreakdown?.games?.length ? (
                <div className="space-y-2">
                  {scoreBreakdown.games.map((g, idx) => (
                    ...
                    >
                      <div>
                        <span className="block font-display font-bold uppercase tracking-wider text-[9px] text-text-muted">
                          MAP {String(g.position ?? idx + 1).padStart(2, "0")}
                        </span>
                ...
                >
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] text-text-muted">
                      Kills:{" "}
                      <span className="text-text-primary">{row.kills}</span>
                    </span>
