				...
				<div className="max-w-7xl mx-auto flex flex-col md:flex-row md:items-center justify-between gap-3 text-xs">
					...
					<div className="flex items-center gap-3 font-data text-[10px] text-text-muted">
						<span className="flex items-center gap-1.5 bg-bg-void px-2 py-1 rounded border border-border-line">
							<span
								className={`w-1.5 h-1.5 rounded-full ${connectionColor}`}
							/>
					...
					<div className="flex items-center justify-between">
						...
						<div className="flex items-center gap-4 w-[40%]">
							...
							<div className="flex flex-col min-w-0">
								{match.team_home_id ? (
									<Link
										href={`/teams/${match.team_home_id}`}
										className="font-display font-black text-lg md:text-2xl text-text-primary leading-tight uppercase truncate hover:text-accent-readout transition-colors"
									>
										{match.team_home?.name}
									</Link>
								) : (
