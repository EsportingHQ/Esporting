...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!isAdmin && !isOrganiser) {
      return new Response(
        JSON.stringify({ error: 'Only organisers and admins can register teams' }),
    ...
    }) => [cg.game_titles.slug, cg.game_titles])
    )
    ...
    if (players && Array.isArray(players) && players.length > 0) {
      for (const playerInput of players) {
        ...
        if (playerGameSlugs && Array.isArray(playerGameSlugs)) {
            ...
            const gameTitle = compGameMap.get(gameSlug) as GameTitle | undefined
