			...
			<main className="max-w-7xl w-full mx-auto px-4 py-8 flex-1 grid grid-cols-1 lg:grid-cols-3 gap-8">
				...
				<section className="lg:col-span-2 space-y-8">
					...
					<div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
						{player.stats.map((stat: { label: string; value: string }) => (
							<div
								key={stat.label}
				...
				<section className="space-y-6">
					<div className="bg-bg-surface border border-border-line rounded p-4 space-y-4">
						...
						{player.gameDisciplines.length === 0 ? (
							<p className="text-xs text-text-muted italic">
								No game assignments yet.
							</p>
						) : (
							player.gameDisciplines.map((g: { name: string }) => (
								<div
									key={g.name}
									className="p-3 bg-bg-void border border-border-line rounded flex items-center justify-between text-xs"
								>
