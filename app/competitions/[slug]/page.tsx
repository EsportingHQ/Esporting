    ...
    return (
      <div className="min-h-screen bg-bg-void text-text-primary">
        ...
        <main className="max-w-7xl mx-auto px-4 py-12">
          <p className="text-sm text-text-muted">Loading competition...</p>
        </main>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-bg-void text-text-primary">
        ...
        <main className="max-w-7xl mx-auto px-4 py-12 space-y-4">
          ...
          <Link href="/competitions" className="text-accent-readout underline">
        ...
        <div className="bg-bg-surface border border-border-line rounded p-6 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            <h1 className="font-display font-black text-3xl tracking-wide uppercase">
              {combineCompetitionName(competition.seriesName, competition.name)}
            </h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div className="md:col-span-2 bg-bg-surface border border-border-line rounded p-6 space-y-4">
                ...
                <p className="text-xs text-text-muted leading-relaxed font-body">
                  Welcome to the flagship competitive arena of Esporting.
                  Matches are conducted live and broadcast directly on the
                  platform dashboard. All stats, scores, and event telemetry
                  update in real-time.
                </p>
                <div className="grid grid-cols-2 gap-4 pt-4 text-xs font-body">
                  ...
                  <div className="bg-bg-void border border-border-line p-3 rounded">
                    ...
                    <span className="font-data">
                      {competition.ends_at
                        ? new Date(competition.ends_at).toLocaleDateString()
                        : "TBD"}
                    </span>
                  </div>
                </div>
            ...
            <div className="space-y-6">
              {isBRFormat ? (
                    ...
                    <div className="p-3 space-y-3 divide-y divide-border-line/50">
                      {stage.matches.map((match) => (
                        <Link
                          ...
                          href={`/competitions/${slug}/matches/${match.id}`}
                        >
                          <MatchCard {...match} showFavorites={false} />
                        </Link>
                      ))}
                    </div>
                  </div>
                ...
                <div className="bg-bg-surface border border-border-line rounded p-10 text-center text-text-muted text-xs">
                  No stage or standings data available for this competition yet.
                </div>
              )}
            </div>
          )}
