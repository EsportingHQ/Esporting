...
export default function AdminMatchesClient({ matches: initialMatches, contributors }: Props) {
  ...
  async function assignContributor(matchId: string, contribId: string) {
    ...
    try {
      ...
      setMatches((prev) =>
        prev.map((m) => (m.id === matchId ? { ...m, contrib_id: contribId || null } : m))
      )
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to assign contributor')
    } finally {
      setSavingId(null)
    }
  }

  const visibleMatches = filter === 'unassigned'
    ? matches.filter((m) => !m.contrib_id)
    : matches

  return (
    <div style={{ maxWidth: 900 }}>
      <h2 style={{ fontSize: 22, fontWeight: 700, marginBottom: 8 }}>Match Assignments</h2>
      ...
      <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
        <button onClick={() => setFilter('unassigned')} style={toggleButtonStyle(filter === 'unassigned')}>
          Unassigned ({matches.filter((m) => !m.contrib_id).length})
        </button>
        <button onClick={() => setFilter('all')} style={toggleButtonStyle(filter === 'all')}>
          All ({matches.length})
        </button>
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
        {visibleMatches.length === 0 && (
          <p style={{ color: '#666', fontSize: 13 }}>
            {filter === 'unassigned' ? 'All matches are assigned.' : 'No matches found.'}
          </p>
        )}
          ...
          >
            <div>
              ...
              <p style={{ fontSize: 12, color: '#888', marginTop: 2 }}>
                ...
                <span style={{ color: match.status === 'live' ? '#22c55e' : '#999' }}>{match.status}</span>
              </p>
            </div>
