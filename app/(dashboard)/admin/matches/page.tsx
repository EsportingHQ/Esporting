...
export default async function AdminMatchesPage() {
  ...
  const { data: matches } = await supabase
    .from('matches')
    .select(`
      id, status, scheduled_at, match_format, contrib_id,
      game_titles(name),
      comp_instances(name),
  ...
  // Fetch the organiser role ID, then contributor role ID separately
  // (avoids the ambiguous FK join issue we hit earlier tonight)
  ...
  // Fetch all users with contributor role
  const { data: contributorAssignments } = await supabase
    .from('user_role_assignments')
    .select('user_id, profiles!user_role_assignments_user_id_fkey(username, display_name)')
    .eq('role_id', contributorRole?.id ?? 0)
    .is('revoked_at', null)

  const contributors = ((contributorAssignments ?? []) as unknown as {
    user_id: string
