  ...
  return {
    ...
    game_titles: first(
      rawMatch.game_titles as
        MatchRoomMatch["game_titles"] | MatchRoomMatch["game_titles"][],
    ),
    comp_instances: first(
      rawMatch.comp_instances as
        MatchRoomMatch["comp_instances"] | MatchRoomMatch["comp_instances"][],
    ),
    comp_stages: first(
      rawMatch.comp_stages as
        MatchRoomMatch["comp_stages"] | MatchRoomMatch["comp_stages"][],
    ),
    home_team: first(
      rawMatch.home_team as
        MatchRoomMatch["home_team"] | MatchRoomMatch["home_team"][],
    ),
    away_team: first(
      rawMatch.away_team as
        MatchRoomMatch["away_team"] | MatchRoomMatch["away_team"][],
    ),
  };
}
...
function normalizeEvents(rows: unknown[] | null): MatchEvent[] {
  return (rows ?? []).map((row) => {
    ...
    return {
      ...
      players: first(
        event.players as MatchEvent["players"] | MatchEvent["players"][],
      ),
    };
  });
}
...
function normalizeParticipants(rows: unknown[] | null): Participant[] {
  return (rows ?? []).map((row) => {
    ...
    return {
      ...
      teams: first(
        participant.teams as Participant["teams"] | Participant["teams"][],
      ),
      players: first(
...
export default async function ContributorMatchRoomPage({
  ...
  const [
    ...
    supabase
      ...
      .in(
        "team_id",
        [match.team_home_id, match.team_away_id].filter(Boolean) as string[],
      )
      .eq("is_active", true),
    supabase
      .from("match_lineups")
