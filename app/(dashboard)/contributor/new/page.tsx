...
export default async function InviteContributorPage() {
  ...
  const organiserCompetitionIds = competitions.map(
  ...
  if (!isAdmin) {
    if (organiserCompetitionIds.length === 0) {
      contributorQuery = contributorQuery.eq(
        "comp_instance_id",
        "00000000-0000-0000-0000-000000000000",
      );
    } else {
      contributorQuery = contributorQuery.in(
        "comp_instance_id",
        organiserCompetitionIds,
      );
    }
  }
  ...
  const existingContributors = (
    ...
    scope_name: contributor.comp_instance_id
      ? (scopeNames.get(contributor.comp_instance_id) ??
