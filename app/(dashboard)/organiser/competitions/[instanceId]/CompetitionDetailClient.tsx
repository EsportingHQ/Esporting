...
import { AlertCircle, Plus, Users, Award, Calendar, Layers, Shield } from 'lucide-react';
  ...
  return (
    <div className="max-w-4xl space-y-6 font-body">
      ...
      <div className="bg-bg-surface border border-border-line rounded p-6 space-y-4">
        ...
        <div className="flex flex-wrap gap-4 text-xs font-data text-text-muted">
          ...
          <span className={`px-2 py-0.5 rounded-full text-[10px] font-display font-bold uppercase tracking-wider ${
            instance.status === 'ongoing' ? 'bg-state-win/10 text-state-win border border-state-win/30' : 'bg-bg-void text-text-muted border border-border-line'
          }`}>
            {instance.status}
          </span>
          {instance.prize_pool && (
        ...
        <div className="flex flex-wrap gap-1.5 pt-1">
          {gameTitles.map((g, i) => (
            <span key={i} className="text-[10px] font-display font-bold bg-bg-void border border-border-line text-text-primary px-2 py-0.5 rounded uppercase">
        ...
        <div className="space-y-4">
          ...
          <div className="space-y-2">
            {localRegistrations.length === 0 && (
              <p className="text-xs text-text-muted italic">No teams registered yet.</p>
            )}
            {localRegistrations.map((reg) => (
              ...
              >
                ...
                <span className={`px-2 py-0.5 rounded text-[10px] font-display font-bold uppercase tracking-wider ${
                  reg.status === 'approved' ? 'bg-state-win/10 text-state-win' : 'bg-bg-void text-text-muted'
                }`}>
                  {reg.status}
                </span>
              </div>
      ...
      {activeTab === 'matches' && (
        <div className="space-y-2">
          {matches.length === 0 && (
            <p className="text-xs text-text-muted italic">No matches scheduled yet.</p>
          )}
          {matches.map((m) => (
            <div
              key={m.id}
            ...
            >
              ...
              <span className={`px-2 py-0.5 rounded text-[10px] font-display font-bold uppercase tracking-wider inline-block ${
                m.status === 'live' ? 'bg-state-win/10 text-state-win' : 'bg-bg-void text-text-muted border border-border-line'
              }`}>
                {m.status}
              </span>
            </div>
