...
function mapTimeLabel(status: string, scheduledAt: string | null): string {
    ...
    if (status === 'scheduled' && scheduledAt) {
        return new Date(scheduledAt).toLocaleString([], { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' });
    }
    return status;
}

function mapRowToCard(row: RawMatchRow, competitionSlug: string): MatchCardProps {
...
export function useCompetitionDetail(slug: string) {
    ...
    try {
        ...
        const seriesName = one(instanceRaw.comp_series as { name: string } | { name: string }[] | null)?.name ?? '';
        ...
        const teams: ParticipantTeam[] = (regs ?? [])
            .map((r: { teams: { id: string; name: string; short_code: string | null } | { id: string; name: string; short_code: string | null }[] | null }) => one(r.teams))
            .filter(Boolean)
        ...
        const upcoming = rows.filter((r) => ['scheduled', 'delayed', 'live'].includes(r.status));
        ...
        if (isBR) {
            ...
            for (const row of (brRows ?? []) as unknown as {
                ...
                kill_pts: number;
                total_pts: number;
                match_participants: { team_id: string | null; teams: { id: string; name: string; short_code: string | null } | { id: string; name: string; short_code: string | null }[] | null } | { team_id: string | null; teams: { id: string; name: string; short_code: string | null } | { id: string; name: string; short_code: string | null }[] | null }[] | null;
                }[]) {
            ...
            const { data: realStandings } = stageIds.length
                ? await supabase
                    .from('standings')
                    .select('rank, played, wins, draws, losses, score_diff, points, teams(id, name, short_code)')
                    .in('stage_id', stageIds)
                    .order('rank', { ascending: true })
                : { data: [] };

            const mapped: StandingRow[] = ((realStandings ?? []) as unknown as {
                rank: number | null;
                played: number;
                wins: number;
                draws: number;
                ...
                score_diff: number;
                points: number;
                teams: { id: string; name: string; short_code: string | null } | { id: string; name: string; short_code: string | null }[] | null;
            }[]).map((row, i) => {
