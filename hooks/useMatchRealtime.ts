  ...
  useEffect(() => {
    ...
    const fetchInitial = async () => {
      ...
      try {
        const { data: matchData, error: matchErr } = await supabase
          .from("matches")
          .select(
            "*, team_home:teams!matches_team_home_id_fkey(*), team_away:teams!matches_team_away_id_fkey(*), game_title:game_titles(*, game_types(*))",
          )
          .eq("id", matchId)
          .single();
    ...
    channel
      .on(
        ...
        (payload) => {
          setMatch((prev) =>
            prev ? { ...prev, ...payload.new } : (payload.new as Match),
          );
        },
      )
      .on(
